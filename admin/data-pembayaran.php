...
<body>
    ...
    <section class="mt-5 mb-5 pt-5">
        ...
        <div class="container table-responsive">
            ...
            <!-- form search -->
            <form style="margin: 5px;" method="GET" action="data-pembayaran.php">
                <div class="row">
            ...
            <table id="example" class="table table-borderless table-dark table-striped table-hover" border="1">
                ...
                <tbody>
                    ...
                    if (isset($_GET['search'])) {
                        ...
                        $list_peserta = mysqli_query($conn, "SELECT * FROM tb_pendaftaran JOIN tb_pembayaran ON tb_pendaftaran.id_pendaftaran = tb_pembayaran.id_pendaftaran WHERE nm_peserta LIKE '%$search%' ORDER BY nm_peserta ASC");
                        $jml_data = mysqli_num_rows($list_peserta);

                        $time_end = microtime(true);
                        $time = $time_end - $time_start;
                    } else {
                        $list_peserta = mysqli_query($conn, "SELECT * FROM tb_pendaftaran JOIN tb_pembayaran ON tb_pendaftaran.id_pendaftaran = tb_pembayaran.id_pendaftaran");
                    }
                    ...
                    ?>
                        <tr>
                            ...
                            <?php if($row['status_pembayaran']== 1){ ?>
                                <td><span class="badge rounded-pill bg-success">Pembayaran Berhasil</span></td>
                            <?php }else if($row['status_pembayaran']== 0 || NULL){ ?>
                                <td><span class="badge rounded-pill bg-primary">Pembayaran di Belum di Konfirmasi / Belum Disetujui</span></td>
                            <?php }else{ ?>
                            <?php } ?>
                            <td class="d-flex gap-2">
                                <a class="btn btn-warning" href="detail-pembayaran.php?id=<?php echo $row['id_pendaftaran'] ?>"><i class="bi bi-bookmark-check-fill"></i> Detail</a>
