...
$peserta = mysqli_query($conn, "SELECT * FROM tb_pendaftaran JOIN tb_pembayaran ON tb_pendaftaran.id_pendaftaran = tb_pembayaran.id_pendaftaran WHERE tb_pendaftaran.id_pendaftaran = '" . $_GET['id'] . "' ");
...
<body>
    ...
    <section class="mt-5 mb-5 pt-5">
        <h2 class="text-center pt-5">Detail Pembayaran</h2>
        <div class="container table-responsive">
            <a href="../user/cetak-pembayaran.php?id=<?php echo $_GET['id'] ?>" target="_blank" class="btn btn-primary mb-2 rounded-pill"><i class="bi bi-printer-fill"></i> Print</a>
            <a href="../admin/konfirmasi.php?id=<?php echo $_GET['id'] ?>" onclick="return confirm('Yakin Terima Pembayaran ?')" class="btn btn-success mb-2 rounded-pill"> Terima Pembayaran</a>
            <a href="../admin/tolak_pembayaran.php?id=<?php echo $_GET['id'] ?>" onclick="return confirm('Yakin Tolak Pembayaran?') " class="btn btn-danger mb-2 rounded-pill"> Tolak Pembayaran</a>
            ...
            <table class="table table-borderless table-striped table-light" border="0">
                ...
                <tr>
                    ...
                    <?php if($p->status_pembayaran == 1):?>
                        ...
                        <td><span class="badge badge-lg rounded-pill bg-success">Pembayaran di Konfirmasi </span></td>
                    <?php else:?>
                        <td><span class="badge badge-lg rounded-pill bg-danger">Pembayaran di Belum di Konfirmasi / Belum Disetujui</span></td>
                        <?php endif;?>
                </tr>
                ...
                <tr>
                    ...
                    <?php if ($p->bukti_pembayaran != NULL) : ?>
                        <td><img src="../user/bukti_pembayaran/<?= $p->bukti_pembayaran ?>" width="150" height="auto" /></td>
                    <?php else : ?>
